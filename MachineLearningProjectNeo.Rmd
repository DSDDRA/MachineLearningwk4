...
# Get understanding of the classe variable in the training data set          
           levels (as.factor(trnNorm$classe))
       
# Split trnNorm into training and Validation at a 70%/30% rate
          inTrain <- createDataPartition(trnNorm$classe, p = 0.7, list = FALSE)
         training <- trnNorm[inTrain, ]
       validation <- trnNorm[-inTrain, ]
```
...
```{r Predict using  Ranger Random Forest}
       ans3<-predict(modFitRRF,tstNorm)
       ans3
       summary(ans3)
```
...
A judgement call was made to normilze the data, while this was not rigoursly tested against non-normailzed data, it appears to be a sound call. The application of simple Trees yielded a prediction capability similar to a coin toss, and thus not a preferred implementation of ML. The Random Forest approach was then applied using the Ranger package, and the accuracy went to 99.7%.  The Ranger package is a rapid implementation of RF, and was implemented for time saving. This model was ultimately used to predict using the testing data.    
